...
/**
 * Institution-scoped Equity Dashboard.
 * Completion rates by demographics, AI insights on equity gaps for the current institution.
 */

import { useState, useEffect } from "react";
import { BarChart3, Sparkles, Users, Loader2 } from "lucide-react";
import { getEquityInsightsForInstitution } from "@/app/actions/admin-equity";
import type { EquityDashboardPayload } from "@/app/api/equity/route";

export default function SisEquityPage() {
  ...
  const handleGenerateInsights = () => {
    ...
    getEquityInsightsForInstitution(summary).then((res) => {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-400 text-sm mt-1">
          Completion rates by demographics and AI insights on equity gaps for your institution.
        </p>
      </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {(
                ...
                <div key={label} className="rounded-lg bg-white/5 p-3">
                  ...
                  <p className="text-slate-500 text-xs mt-0.5">{d.completed} completed</p>
                </div>
              ))}
            </div>
          </div>
