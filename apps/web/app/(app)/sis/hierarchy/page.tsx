...
/**
 * Institution-scoped Organizational Hierarchy Builder.
 * Tree view: Schools (when enabled) or Faculties → Departments → Programmes.
 * Uses current tenant from org context (/api/hierarchy).
 */
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 py-2 group">
        ...
        <button type="button" onClick={() => setOpen((o) => !o)} className="p-0.5 rounded hover:bg-white/10 text-slate-400">
...
function SchoolBlock({
  ...
  return (
    <div
      ...
      onDrop={handleDrop}
    >
      <div className="flex items-center gap-3 px-4 py-3 border-b border-white/10 bg-white/5 cursor-pointer" onClick={() => setOpen((o) => !o)}>
        ...
        {open && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="px-4 pb-4">
            {s.departments.length === 0 ? (
              ...
              s.departments.map((d) => (
                <DepartmentRow key={d.id} d={d} departmentIds={departmentIds} onReorder={onReorderDepartments} canEdit={canEdit} />
              ))
            )}
          </motion.div>
        )}
      ...
      <DialogContent className="max-w-md border-white/10 bg-space-800 text-slate-200">
        ...
        <div className="space-y-4 pt-2">
          ...
          <div className="space-y-2">
            ...
            <Input value={memberSearch} onChange={(e) => setMemberSearch(e.target.value)} placeholder="Search by name or email..." className="border-white/20 bg-transparent text-slate-200 mb-2" />
...
export default function SisHierarchyPage() {
    ...
    <div className="min-h-screen bg-grid-pattern bg-space-950">
      <div className="mb-8">
        ...
        <p className="text-slate-400 mt-1">
          {data?.schoolsEnabled
            ? "Schools → Departments → Programmes. Drag the grip to reorder; add or edit below."
            ...
            <div className="space-y-0">
              {data.schools.map((s) => (
                <SchoolBlock
                  ...
                  onAddDepartment={() => setDepartmentModal({ schoolId: s.id, facultyId: undefined })}
