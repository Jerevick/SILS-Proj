...
/**
 * Phase 17: Create new announcement with scope picker.
...
export default function CreateAnnouncementPage() {
  ...
  const filteredOptions: AnnouncementScopeOption[] =
    scopeData?.filter((o) => o.type === targetScopeType) ?? [];
  ...
  return (
    <AdminShell activeNav="announcements">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(v) => setTargetScopeType(v as AnnouncementScopeType)}
                >
                  <SelectTrigger className="border-white/20 bg-transparent text-slate-200">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    {(Object.entries(SCOPE_TYPE_LABELS) as [AnnouncementScopeType, string][]).map(
                      ([value, label]) => (
              ...
              {targetScopeType !== "ALL" && (
                <div className="space-y-2">
                  <Label className="text-slate-300">
                    {SCOPE_TYPE_LABELS[targetScopeType]}
                  </Label>
                  <Select
                    value={targetScopeId}
                    onValueChange={setTargetScopeId}
                    disabled={loadingScope || filteredOptions.length === 0}
                  >
                    <SelectTrigger className="border-white/20 bg-transparent text-slate-200">
