...
/**
 * Phase 14: Visual Workflow Builder — Configurable multi-step admissions workflows.
 ...
 * Platform admin: select institution (tenant) then configure workflows per programme type.
 */
...
// Default templates: steps per programme type (Admissions → Academic Affairs → HoD → Dean → Registry)
...
type InstitutionRow = { id: string; name: string; slug: string };

async function fetchInstitutions(): Promise<InstitutionRow[]> {
  const res = await fetch("/api/admin/institutions");
  if (!res.ok) throw new Error("Failed to fetch institutions");
  const data = await res.json();
  return data.map((t: { id: string; name: string; slug: string }) => ({
    id: t.id,
    name: t.name,
    slug: t.slug,
  }));
...
async function fetchWorkflows(tenantId: string) {
  const res = await fetch(`/api/admin/workflows?tenantId=${encodeURIComponent(tenantId)}`);
...
export default function AdminWorkflowsPage() {
  ...
  const { data: workflows = [], isLoading: workflowsLoading } = useQuery({
    queryKey: ["admin-workflows", tenantId],
  ...
  // Sync edits from server when workflows load (once per tenant)
  useEffect(() => {
    if (!tenantId || workflows.length === 0 || workflowsLoading) return;
    if (hydratedForRef.current === tenantId) return;
    hydratedForRef.current = tenantId;
    setEdits((prev) => {
      const next = { ...prev };
  ...
  }, [tenantId, workflows, workflowsLoading]);
  ...
  const saveMutation = useMutation({
    mutationFn: ({
      ...
      name: string;
      steps: StepEdit[];
    }) => saveWorkflow(tenantId, programmeType, name, steps),
    onSuccess: (_, { programmeType }) => {
      queryClient.invalidateQueries({ queryKey: ["admin-workflows", tenantId] });
          ...
          <Card sx={{ bgcolor: "rgba(15,15,35,0.8)", border: "1px solid rgba(255,255,255,0.1)" }}>
            <CardContent sx={{ pt: 2 }}>
              <FormControl size="small" sx={{ minWidth: 280 }}>
                <InputLabel id="tenant-label">Institution (tenant)</InputLabel>
                <Select
                  labelId="tenant-label"
                  value={tenantId}
                  label="Institution (tenant)"
            ...
            <Card sx={{ bgcolor: "rgba(15,15,35,0.8)", border: "1px solid rgba(255,255,255,0.1)" }}>
              <CardContent>
                <Typography color="text.secondary">
                  Select an institution above to configure admission workflows per programme type.
                </Typography>
              </CardContent>
            </Card>
          ) : (
